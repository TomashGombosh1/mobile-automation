 ...
 */
public class LogoutScreen extends AbstractScreen {
    private static final AppElement SCREEN_TITLE = new AppElement(
            "Screen title",
            By.id(formatAndroidId("toolbar_title")),
            MobileBy.iOSNsPredicateString("type == 'XCUIElementTypeStaticText' AND value == 'Logout'"),
            ScrollTo.NO,
            true);

    public LogoutScreen(final AppiumDriver<MobileElement> driver) {
        super(driver);
    }
